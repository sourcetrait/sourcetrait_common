...
r#"use ::asmov_common_traitenum_macro::enumtrait;
...
r#"::asmov_common_traitenum_parse::gen_require!(%{LIB_CRATE_NAME}%, %{DERIVE_CRATE_NAME}%);
"#;
...
fn make_derive(library: &cli::WorkspaceCommand) -> anyhow::Result<()> {
    ...
    dbg!(lib_name);
    dbg!(lib_name.to_case(case::Case::Snake));
...
fn config_lib(library: &cli::WorkspaceCommand) -> anyhow::Result<()> {
    ...
    let traitenum_crate_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("traitenum-macro");

    let traitenum_macro_crate_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("..")
            .join("traitenum");

    cargo_add(&lib_path, None, Some(&traitenum_crate_path))?;
    cargo_add(&lib_path, None, Some(&traitenum_macro_crate_path))?;
...
fn config_derive(library: &cli::WorkspaceCommand) -> anyhow::Result<()> {
    ...
    //todo
    let traitenum_parse_crate_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("traitenum-parse");

    cargo_add(&derive_path, Some("proc-macro2"), None)?;
    cargo_add(&derive_path, None, Some(&traitenum_crate_path))?;
    cargo_add(&derive_path, None, Some(&traitenum_parse_crate_path))?;
