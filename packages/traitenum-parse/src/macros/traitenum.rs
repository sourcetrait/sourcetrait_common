...
pub(crate) fn parse_traitenum_macro(
    ...
    let input_scope = &input.vis;
    let input_iterator_ident = syn::Ident::new(&format!("{}Iterator", input_ident), span(&input));
    ...
    let next_ordinal_match_body = traitenum.variants().iter().map(|variant| {
        ...
        let output = quote::quote!{
            #ordinal => ::std::option::Option::Some(#item_path::#variant_ident),
        };
    ...
    let output = quote::quote!{
        ...
        impl EnumTrait for #input_ident {
            type Iterator = #input_iterator_ident;

            fn iter() -> Self::Iterator {
                #input_iterator_ident::new()
            }
        }

        #input_scope struct #input_iterator_ident {
            next_ordinal: usize
        }

        impl #input_iterator_ident {
            fn new() -> Self { Self { next_ordinal: 0 } }
        }

        impl ::std::iter::Iterator for #input_iterator_ident {
            type Item = #input_ident;
