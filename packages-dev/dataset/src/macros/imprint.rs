...
use crate::dataset::strategy::DatasetType;
pub use crate::*;
...
macro_rules! imprint_meta_for_model {
    ($Model:ty, $schema_name:literal, $schema_name_plural:literal) => {
        impl ::asmov_common_dataset::MetaModel for $Model {
        ...
        impl ::asmov_common_dataset::MetaModelMut for $Model {
...
/*
pub async fn default_strategic_dataset_get<'d: 'm, 'm, MEM, M>(
    dataset: &'d StrategicDataset<MEM>,
    id: ID,
) -> Result<Option<std::borrow::Cow<'m, M>>>
where
    MEM: MemoryDataset,
    M: StrategicDatasetModel<MEM> + 'm,
{
    for dataset_type in dataset.options.get_order {
        match dataset_type {
            DatasetType::Memory => {
                let result = M::dataset_get(dataset.memory_dataset.as_ref().unwrap(), id).await?;
...
macro_rules! imprint_strategic_dataset_for_model {
    ($Model:ty) => {
        impl DatasetModel<StrategicDataset> for $Model {
            async fn dataset_get<'d: 'm, 'm>(
                dataset: &'d StrategicDataset,
                id: ID,
            ) -> Result<Option<Cow<'m, Self>>>
            where
                Self: 'm,
            {
                default_strategic_dataset_get(dataset, id).await
            }
            ...
            async fn dataset_put<'m>(dataset: &'m mut StrategicDataset, model: Self) -> Result<ID>
            where
                Self: 'm,
            {
                todo!()
        ...
        impl DatasetModel<$MemoryDataset> for $Model {
                ...
                Self: 'm,
            {
                Ok(dataset
                    .$model_variable
                    .get(&id)
                    .and_then(|m| Some(Cow::Borrowed(m))))
            }

            async fn dataset_get_mut<'d: 'm, 'm>(
                dataset: &'d mut MemoryDataset,
                id: ID,
            ) -> Result<Option<&'m mut Self>>
            where
                Self: 'm,
            {
            ...
            async fn dataset_put<'m>(dataset: &'m mut MemoryDataset, model: Self) -> Result<ID>
            where
                Self: 'm,
            {
...
macro_rules! imprint_sql_dataset_for_model {
    ($Dataset:ty, $Model:ty) => {
        impl DatasetModel<$Dataset> for $Model {
            ...
            async fn dataset_put<'a>(_dataset: &'a mut $Dataset, _model: Self) -> Result<ID>
            where
                Self: 'a,
            {
                todo!()
