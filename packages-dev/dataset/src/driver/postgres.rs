...
impl PostgresDataset {
        ...
        for<'r> M: sqlx::FromRow<'r, postgres::PgRow> + Unpin + 'r,
    {
        ...
        let (id_field, id) = match id.into_option() {
            ...
            OptionID::Some(ID::Authorative(id)) | OptionID::Some(ID::Mutual(_, id)) => {
                (FIELD_ID, id as i64)
            }
            OptionID::Some(ID::Local(_)) if self.authorative => {
                panic!("Cannot PostgreSQL SELECT for a local ID from an authorative dataset")
            }
        ...
        let result = sqlx::query_as::<_, M>(&format!(
            "SELECT * FROM {table} WHERE {id_field} = ? LIMIT 1"
        ))
        .bind(id)
        .fetch_one(&self.pool)
        .await;
