...
async fn main() {
    ...
    let req = green::Packet::duplex(ToExampleSys::MathRequest(
        MathRequest {
            operand: 2,
        }
    ));
    sys.send_packet(req).await.unwrap();
    let expected = green::MsgFromSys::Packet(req.respond(
        FromExampleSys::MathResponse(MathResponse {
            result: Ok(2),
        }),
    ));
    let actual = sys.recv().await.unwrap();
    ...
    sys.send_packet(req).await.unwrap();
    let expected = green::MsgFromSys::Packet(req.respond(FromExampleSys::MathResponse(
        MathResponse {
            result: Ok(6),
        }
    )));
...
impl green::PacketData for MathRequest {}
...
impl green::System for ExampleSys {
    ...
    async fn on_channel_recv(&mut self, pkt: green::Packet<ToExampleSys>) -> green::FlowResult<Self::Flow> {
        let (rqid, msg) = pkt.into_tuple();
        match msg {
            ToExampleSys::MathRequest(rq) => self.on_math_request(rq.into_packet(rqid)).await,
        }
    }
...
impl ExampleSys {
    async fn on_math_request(&mut self, pkt: green::Packet<MathRequest>) -> green::FlowResult<green::StdFlow> {
        self.num = match self.op {
            Operation::Add => pkt.msg.operand + self.num,
            Operation::Mul => pkt.msg.operand * self.num,
        };
        
        self.send_channel_packet(pkt.respond(FromExampleSys::MathResponse(
            MathResponse {
                result: Ok(self.num),
            }
        ))).await?;
